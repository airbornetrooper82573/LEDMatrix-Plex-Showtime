"""Plex Showtime v1.1.2 for ChuckBuilds LEDMatrix.

Robust Plex XML parsing, useful diagnostics, and a 320x32-friendly marquee
layout for five chained 64x32 panels.
"""

from __future__ import annotations

import io
import random
import time
import urllib.parse
import urllib.request
import xml.etree.ElementTree as ET
from dataclasses import dataclass
from pathlib import Path
from typing import List, Optional

from PIL import Image, ImageDraw, ImageFont, ImageOps
from src.plugin_system.base_plugin import BasePlugin


@dataclass
class PlexItem:
    source: str
    media_type: str
    title: str
    parent_title: str = ""
    grandparent_title: str = ""
    year: str = ""
    summary: str = ""
    thumb: str = ""
    art: str = ""
    index: str = ""
    parent_index: str = ""

    @property
    def display_title(self) -> str:
        if self.media_type == "episode" and self.grandparent_title:
            return self.grandparent_title
        return self.title or self.parent_title or self.grandparent_title or "Plex"

    @property
    def subtitle(self) -> str:
        if self.media_type == "episode":
            bits = []
            if self.parent_index and self.index:
                try:
                    bits.append(f"S{int(self.parent_index):02d}E{int(self.index):02d}")
                except (TypeError, ValueError):
                    pass
            if self.title:
                bits.append(self.title)
            return " • ".join(bits)
        if self.media_type in ("track", "album") and self.parent_title:
            return self.parent_title
        return self.year


class PlexShowtimePlugin(BasePlugin):
    def __init__(self, plugin_id, config, display_manager, cache_manager, plugin_manager):
        super().__init__(plugin_id, config, display_manager, cache_manager, plugin_manager)

        self.plex_url = str(config.get("plex_url", "")).rstrip("/")
        self.plex_token = str(config.get("plex_token", "")).strip()
        self.show_playing = bool(config.get("show_playing", True))
        self.show_recently_added = bool(config.get("show_recently_added", True))
        self.show_recently_played = bool(config.get("show_recently_played", True))
        self.show_library = bool(config.get("show_library", True))
        self.filter_movies = bool(config.get("filter_movies", True))
        self.filter_tv = bool(config.get("filter_tv", True))
        self.filter_music = bool(config.get("filter_music", False))
        self.show_heading = bool(config.get("show_heading", True))
        self.show_year = bool(config.get("show_year", True))
        self.show_summary = bool(config.get("show_summary", False))
        self.display_layout = str(config.get("layout", "auto"))
        self.artwork_mode = str(config.get("artwork_mode", "auto"))
        self.recent_count = max(1, int(config.get("recent_count", 20)))
        self.live_priority = bool(config.get("live_priority", False))
        self.artwork_width = max(32, int(config.get("artwork_width", 72)))
        self.accent_width = max(1, int(config.get("accent_width", 2)))
        self.show_panel_dividers = bool(config.get("show_panel_dividers", False))
        self.panel_width = max(1, int(config.get("panel_width", 64)))
        self.title_font_size = max(6, int(config.get("title_font_size", 11)))
        self.subtitle_font_size = max(5, int(config.get("subtitle_font_size", 7)))
        self.heading_font_size = max(5, int(config.get("heading_font_size", 6)))

        self.W = int(display_manager.width)
        self.H = int(display_manager.height)
        self.current_item: Optional[PlexItem] = None
        self.current_image: Optional[Image.Image] = None
        self.last_error = ""
        self.last_update = 0.0
        self._font_heading = self._load_font(self.heading_font_size)
        self._font_subtitle = self._load_font(self.subtitle_font_size)
        self._font_title = self._load_font(self.title_font_size)

        self.logger.info(
            "Plex Showtime v1.1.2 initialized: %dx%d url=%s filters(movie=%s tv=%s music=%s)",
            self.W, self.H, self.plex_url or "<missing>",
            self.filter_movies, self.filter_tv, self.filter_music,
        )

    def _load_font(self, size: int):
        for p in (
            Path("assets/fonts/PressStart2P-Regular.ttf"),
            Path("/home/ledpi/LEDMatrix/assets/fonts/PressStart2P-Regular.ttf"),
        ):
            if p.exists():
                try:
                    return ImageFont.truetype(str(p), size)
                except Exception:
                    pass
        return ImageFont.load_default()

    def _request_xml(self, path: str) -> Optional[ET.Element]:
        if not self.plex_url or not self.plex_token:
            self.last_error = "Configure Plex URL/token"
            return None

        url = f"{self.plex_url}{path}"
        req = urllib.request.Request(
            url,
            headers={
                "X-Plex-Token": self.plex_token,
                "Accept": "application/xml,text/xml,*/*",
                "User-Agent": "LEDMatrix-Plex-Showtime/1.1.2",
                "X-Plex-Product": "LEDMatrix Plex Showtime",
                "X-Plex-Version": "1.1.2",
                "X-Plex-Client-Identifier": "ledmatrix-plex-showtime",
            },
        )
        try:
            with urllib.request.urlopen(req, timeout=8) as response:
                raw = response.read()
            root = ET.fromstring(raw)
            self.logger.debug(
                "Plex %s -> root=%s size=%s direct_children=%d",
                path, root.tag, root.get("size", "?"), len(list(root)),
            )
            return root
        except Exception as exc:
            self.last_error = f"Plex request failed: {exc}"
            self.logger.warning("Plex request failed for %s: %s", path, exc)
            return None

    def _allowed_type(self, media_type: str) -> bool:
        media_type = (media_type or "").lower()
        if media_type == "movie":
            return self.filter_movies
        if media_type in ("show", "season", "episode", "clip"):
            return self.filter_tv
        if media_type in ("artist", "album", "track"):
            return self.filter_music
        return False

    def _to_item(self, element: ET.Element, source: str) -> Optional[PlexItem]:
        media_type = (element.get("type") or "").lower()
        if not self._allowed_type(media_type):
            return None
        title = (element.get("title") or "").strip()
        grandparent = (element.get("grandparentTitle") or "").strip()
        if not title and not grandparent:
            return None
        return PlexItem(
            source=source,
            media_type=media_type,
            title=title,
            parent_title=(element.get("parentTitle") or "").strip(),
            grandparent_title=grandparent,
            year=(element.get("year") or "").strip(),
            summary=(element.get("summary") or "").strip(),
            thumb=(element.get("thumb") or "").strip(),
            art=(element.get("art") or "").strip(),
            index=(element.get("index") or "").strip(),
            parent_index=(element.get("parentIndex") or "").strip(),
        )

    def _items_from_root(self, root: Optional[ET.Element], source: str) -> List[PlexItem]:
        if root is None:
            return []

        items: List[PlexItem] = []
        seen = set()
        raw_types = {}

        # Recursive iteration is intentional: some Plex endpoints return media
        # directly under MediaContainer while others can wrap it more deeply.
        for element in root.iter():
            media_type = (element.get("type") or "").lower()
            if media_type:
                raw_types[media_type] = raw_types.get(media_type, 0) + 1
            item = self._to_item(element, source)
            if not item:
                continue
            key = (
                item.media_type, item.title, item.grandparent_title,
                item.parent_index, item.index,
            )
            if key in seen:
                continue
            seen.add(key)
            items.append(item)
            if len(items) >= self.recent_count:
                break

        self.logger.info(
            "Plex Showtime source '%s': Plex types=%s usable=%d",
            source, raw_types or "none", len(items),
        )
        return items

    def _playing_items(self) -> List[PlexItem]:
        if not self.show_playing:
            return []
        return self._items_from_root(self._request_xml("/status/sessions"), "NOW PLAYING")

    def _recently_added_items(self) -> List[PlexItem]:
        if not self.show_recently_added:
            return []
        return self._items_from_root(self._request_xml("/library/recentlyAdded"), "RECENTLY ADDED")

    def _recently_played_items(self) -> List[PlexItem]:
        if not self.show_recently_played:
            return []
        return self._items_from_root(
            self._request_xml("/status/sessions/history/all?sort=viewedAt%3Adesc"),
            "RECENTLY PLAYED",
        )

    def _library_items(self) -> List[PlexItem]:
        if not self.show_library:
            return []
        root = self._request_xml("/library/sections")
        if root is None:
            return []

        sections = []
        for el in root.iter():
            t = (el.get("type") or "").lower()
            key = el.get("key") or ""
            if not key:
                continue
            if t == "movie" and self.filter_movies:
                sections.append((key, t))
            elif t == "show" and self.filter_tv:
                sections.append((key, t))
            elif t == "artist" and self.filter_music:
                sections.append((key, t))

        self.logger.info("Plex Showtime library sections eligible=%s", sections)
        random.shuffle(sections)
        for key, section_type in sections:
            if section_type == "show":
                path = f"/library/sections/{key}/all?type=4"
            else:
                path = f"/library/sections/{key}/all"
            items = self._items_from_root(self._request_xml(path), "FROM YOUR LIBRARY")
            if items:
                return items
        return []

    def _choose_item(self) -> Optional[PlexItem]:
        playing = self._playing_items()
        if playing:
            self.logger.info("Plex Showtime selected active playback")
            return random.choice(playing)

        getters = []
        if self.show_recently_added:
            getters.append(self._recently_added_items)
        if self.show_recently_played:
            getters.append(self._recently_played_items)
        if self.show_library:
            getters.append(self._library_items)
        random.shuffle(getters)

        for getter in getters:
            items = getter()
            if items:
                choice = random.choice(items)
                self.logger.info("Plex Showtime selected '%s' from %s", choice.display_title, choice.source)
                return choice

        if not self.last_error:
            self.last_error = "Plex connected, but no enabled media matched filters"
        return None

    def _image_path(self, item: PlexItem) -> str:
        if self.artwork_mode == "poster":
            return item.thumb or item.art
        if self.artwork_mode == "background":
            return item.art or item.thumb
        return item.thumb or item.art

    def _load_image(self, item: PlexItem) -> Optional[Image.Image]:
        path = self._image_path(item)
        if not path:
            return None
        url = path if path.startswith(("http://", "https://")) else f"{self.plex_url}{path}"
        req = urllib.request.Request(
            url,
            headers={
                "X-Plex-Token": self.plex_token,
                "User-Agent": "LEDMatrix-Plex-Showtime/1.1.2",
            },
        )
        try:
            with urllib.request.urlopen(req, timeout=10) as response:
                return Image.open(io.BytesIO(response.read())).convert("RGB")
        except Exception as exc:
            self.logger.warning("Plex artwork failed for %s: %s", item.display_title, exc)
            return None

    def update(self):
        self.last_update = time.time()
        self.last_error = ""
        if not self.plex_url or not self.plex_token:
            self.current_item = None
            self.current_image = None
            self.last_error = "Configure Plex URL/token"
            return
        try:
            self.current_item = self._choose_item()
            self.current_image = self._load_image(self.current_item) if self.current_item else None
        except Exception as exc:
            self.current_item = None
            self.current_image = None
            self.last_error = f"Plex Showtime error: {exc}"
            self.logger.error("Update failed: %s", exc, exc_info=True)

    def has_live_content(self) -> bool:
        if not self.live_priority or not self.show_playing:
            return False
        return bool(self._playing_items())

    @staticmethod
    def _text_width(draw, text, font):
        box = draw.textbbox((0, 0), text, font=font)
        return max(0, box[2] - box[0])

    def _fit_text(self, draw, text, font, width):
        text = (text or "").strip()
        if self._text_width(draw, text, font) <= width:
            return text
        suffix = "..."
        while text and self._text_width(draw, text + suffix, font) > width:
            text = text[:-1]
        return text.rstrip() + suffix if text else ""

    @staticmethod
    def _cover(image, size):
        return ImageOps.fit(image, size, method=Image.Resampling.LANCZOS, centering=(0.5, 0.5))

    @staticmethod
    def _contain(image, size):
        canvas = Image.new("RGB", size, "black")
        img = image.copy()
        img.thumbnail(size, Image.Resampling.LANCZOS)
        canvas.paste(img, ((size[0] - img.width) // 2, (size[1] - img.height) // 2))
        return canvas

    def _render_marquee(self, item: PlexItem):
        canvas = Image.new("RGB", (self.W, self.H), "black")
        draw = ImageDraw.Draw(canvas)
        art_w = min(max(self.H, self.artwork_width), max(self.H, self.W - 120))

        if self.current_image:
            art = self._contain(self.current_image, (art_w, self.H)) if self.current_image.height >= self.current_image.width else self._cover(self.current_image, (art_w, self.H))
            canvas.paste(art, (0, 0))

        draw.rectangle((art_w, 0, min(self.W - 1, art_w + self.accent_width - 1), self.H - 1), fill=(229, 160, 13))
        x = art_w + self.accent_width + 6
        w = max(1, self.W - x - 5)
        y = 1

        if self.show_heading:
            draw.text((x, y), self._fit_text(draw, item.source, self._font_heading, w), font=self._font_heading, fill=(229, 160, 13))
            y += self.heading_font_size + 2

        draw.text((x, y), self._fit_text(draw, item.display_title, self._font_title, w), font=self._font_title, fill="white")
        y += self.title_font_size + 2

        label = {"movie":"MOVIE","show":"TV","season":"TV","episode":"TV","clip":"TV","artist":"MUSIC","album":"MUSIC","track":"MUSIC"}.get(item.media_type, item.media_type.upper())
        meta = [label] if label else []
        if item.media_type == "episode" and item.subtitle:
            meta.append(item.subtitle)
        elif self.show_year and item.year:
            meta.append(item.year)
        elif item.subtitle:
            meta.append(item.subtitle)
        if meta and y < self.H:
            draw.text((x, y), self._fit_text(draw, "  |  ".join(meta), self._font_subtitle, w), font=self._font_subtitle, fill=(190,190,190))

        if self.show_panel_dividers:
            for px in range(self.panel_width, self.W, self.panel_width):
                draw.line((px, 0, px, self.H - 1), fill=(20,20,20))
        return canvas

    def _render(self):
        if self.current_item is None:
            canvas = Image.new("RGB", (self.W, self.H), "black")
            draw = ImageDraw.Draw(canvas)
            msg = self.last_error or "No Plex media found"
            draw.text((3, max(0, self.H // 2 - 4)), self._fit_text(draw, msg, self._font_subtitle, self.W - 6), font=self._font_subtitle, fill="white")
            return canvas
        return self._render_marquee(self.current_item)

    def display(self, force_clear=False):
        try:
            if force_clear:
                self.display_manager.clear()
            if self.current_item is None:
                self.update()
            self.display_manager.image.paste(self._render(), (0, 0))
            self.display_manager.update_display()
            return True
        except Exception as exc:
            self.logger.error("Display failed: %s", exc, exc_info=True)
            return False

    def validate_config(self):
        if not super().validate_config():
            return False
        return True

    def get_info(self):
        info = super().get_info()
        info.update({
            "last_update": self.last_update,
            "last_error": self.last_error,
            "current_title": self.current_item.display_title if self.current_item else "",
            "current_source": self.current_item.source if self.current_item else "",
        })
        return info
